...
class EDFXClient():
    ...
    #20 minutes for bearer token to be alive.
    ...
    def request_new_bearer_token(self):
        ...
        if response.status_code != 200:
            ...
            err = f"Error in response. Status code: {response.status_code}, message: {response.reason}, detail: {response_detail}"
    ...
    def update_bearer_token_claimset_expiration_info(self):
        ...
        try:
            ...
            raise AuthenticationError(f"Error decoding auth token: {format_exc()}")
    ...
    def revoke_bearer_token(self):
        ...
        """
        # Deleting bearer token doesn't work so we skip it
    ...
    def is_bearer_token_renewal(self):
        ...
        if time_left.total_seconds() < EDFXClient.AUTH_TOKEN_RENEWAL_THRESHOLD_IN_SECONDS:
            return True
    ...
    def ping(self):
        ...
        if response.ok:
            logger.info(f"Single Sign-on (SSO) service connectivity test to '{self.base_url}' - PASSED")
            return True
        else:
            logger.error(
