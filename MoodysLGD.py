...
class LGD(EDFXEndpoints):
    ...
    LGDCASE3Parameters:
        ...
        How would a temporal case look like when I need a Time Series?
            ...
            Example of Temporal 1:
                ...
                If the Loan is using TODAYS Macro Scenario with a loan starting 3 years ago from January 2024 you can pull the last three years LGD via
                filling in the relevant loan paramaters and the following start, end, and history frequency dates.
                Case3LGDOriginationStartDate = "2021-01-01", Case3LGDOriginationEndDate = "2024-01-01",  Case3LGDhistoryFrequency = "Annual"


            Example Temporal 2:

                If the Loan is using a Macro Scenario 3 years ago for a loan starting 3 years ago from January 2024 you can pull the LGD content via
                filling in the relevant loan paramaters and the following start, end, and history frequency dates and asOfDate via
                Case3LGDOriginationStartDate = "2021-01-01", Case3LGDOriginationEndDate = "2022-01-01",
                Case3LGDhistoryFrequency = "Annual", Case3LGDasOfDate = "2021-01-01"
    ...
    def __init__(self,df:pd.DataFrame = None,entities:list[dict[str,str]]=None, case:int=2,Case3LGDInstrumentType:str='Senior Bond',
                    ...
                    Case3LGDOriginationStartDate:str=None, Case3LGDOriginationEndDate:str=pd.Timestamp.now().strftime('%Y-%m-%d'),
        ...
        if self.case == 3:
            ...
            self.Case3LGDOriginationStartDate = Case3LGDOriginationStartDate
            self.Case3LGDOriginationEndDate = Case3LGDOriginationEndDate
    ...
    def LGDClientSideEDFXPDTermStructures(self,CleanedDF:pd.DataFrame,asReported:bool=False,timeout:int=900,PDcompute:int=100,
        ...
        if case == 3:
            ...
            startDate = self.Case3LGDOriginationStartDate
            endDate = self.Case3LGDOriginationEndDate
        else:
    ...
    def EDFXlgd_function(self, df: pd.DataFrame, cumulative:bool=True, use_loan_scorecard:bool=False, user_defined_loan_scorecard:loan_scorecard.LoanScorecard=None):
        ...
        def convert_to_payload(row, idx):
            ...
            loan_parameters = {
                ...
                "originationDate": row.get('originationDate') or self.Case3LGDasOfDate,
