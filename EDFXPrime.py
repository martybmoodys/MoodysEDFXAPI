...
class EDFXEndpoints(EDFXClient):
    ...
    async def EDFXPD_Endpoint_async(self, entities:list[dict[str,str]]=None, startDate:str=None, endDate:str=None, historyFrequency:str='monthly',
        ...
        NOT COMPLETE AS OF 12/29/2023 DO NOT USE YET
        ...
        async def _post_async(params):
            async with session.post(url, headers=headers, json=params) as response:
                ...
                if payload.get('message'):
                    print(f"I'm sorry but the API Returned {payload}. Please re-check your input variables.")
                else:
                    return payload
        ...
        async with aiohttp.ClientSession(timeout=ClientTimeout(total=timeout)) as session:
            ...
            for ii in range(retries, 0, -1):
                try:
                    ...
                    print(f'Batch call failed, {ii} retries left')
    ...
    async def SynchronousBatchMVP_async(self,EntityPayload:list[dict[str,str]], BatchSize:int,historyFrequency:str='monthly',startDate:str=None,
        ...
        for pd_dict in responses:
            if pd_dict is None:
                logger.warning(f"API returned a NONE response.")
                continue
            try:
                # parse the dictionary object to a pandas dataframe
                pd_df = self.EDFXPDParse(pd_dict)
