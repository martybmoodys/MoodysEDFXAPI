...
def PDHistory_extract_ratios(entity: dict) -> list:
    """
...
class EDFXEndpoints(EDFXClient):
    ...
    def EDFXPD_History(self, entities:list[dict[str,str]], historyFrequency:str="monthly" ,startDate:str=None, endDate:str=None,
    ...
    def EDFXPDParse(PDsJSON:dict, FormatType = 'Long', output_format='Pandas', TimeSeries:bool = False,
        ...
        if TimeSeries:
            if FormatType == "Wide":
                df = pd.DataFrame([flatten_dict(e) for e in PDsJSON['entities']])
                ...
                for entity in PDsJSON['entities']:
                    if 'history' in entity:
                        ...
                        history = entity_copy.pop('history')
                        flat_row = flatten_dict(entity_copy)
                        for history_item in history:
                            flat_history = flatten_dict(history_item)
                            flat_history_with_prefix = {f'history_{key}': value for key, value in flat_history.items()}
                            flat_row_copy = flat_row.copy()
                            flat_row_copy.update(flat_history_with_prefix)
                            rows.append(flat_row_copy)
                    else:
                        rows.append(flatten_dict(entity))
    ...
    @staticmethod
    def EDFXPDHistoryParse(json:dict, output_format:str = 'pandas', file_name="pdHistory"):
        ...
        for entity in json['entities']:
            all_ratios.extend(PDHistory_extract_ratios(entity))
    ...
    def EDFXParseTradeCredit(json:dict, output_format:str = "pandas", file_name = "TradeCredit"):
        ...
        if output_format == OutputFormat.PANDAS.value:
            ...
            # You can change the file name as needed or even pass it as a parameter
            full_df.to_excel(f"{file_name}.xlsx", index=False)
            print(f"Data saved to {file_name}.xlsx")
            return None

        elif output_format == OutputFormat.CSV.value:
            full_df.to_csv(f'{file_name}.csv',index=False)
            print(f'Data Saved to {file_name}.csv')
            return None
        else:
            raise ValueError(f"Unsupported OutputFormat: {output_format}")
    ...
    @staticmethod
    def EDFXRatioCalculationsParse(RatiosJSON: dict, FormatType: str = 'Long', output_format='Pandas',
        ...
        flat_data = []
        if FormatType == 'Long':
            for ratio in RatiosJSON['ratios']:
