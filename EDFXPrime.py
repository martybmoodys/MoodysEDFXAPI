...
class EDFXEndpoints(EDFXClient):
    ...
    @staticmethod
    def EDFXPDParse(data: dict, TimeSeries: bool = True, output_format:str = "pandas", file_name = "pd") -> pd.DataFrame:
    ...
    def EDFXStatementsParse(StatementsJSON: dict, FormatType: str = 'Long', output_format='Pandas',
        ...
        if 'message' in StatementsJSON:
            return f"Error in received data: {StatementsJSON['message']}"

        # Make sure 'entities' key exists in StatementsJSON
        if 'entities' not in StatementsJSON:
